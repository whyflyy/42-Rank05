...
/*   Updated: 2025/07/21 14:15:29 by jcavadas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Bureaucrat::executeForm(AForm const &toExecute)
{
	...
	{
		if (this->getGrade() <= toExecute.getExecGrade())
		{
			std::cout << PEACH << this->getName() << GREEN " executed " << MAGENTA << toExecute.getName() << RESET << std::endl;
			toExecute.execute(*this);
		}
		else
		{
			std::cout << PEACH << this->getName() << RED << " doesn't have the grade required to execute " \
			<< MAGENTA << toExecute.getName() << RED << "!" << RESET << std::endl;
		}
	}
	else
		std::cout << PEACH << toExecute.getName() << RED << " is not signed!" << RESET << std::endl;
}
